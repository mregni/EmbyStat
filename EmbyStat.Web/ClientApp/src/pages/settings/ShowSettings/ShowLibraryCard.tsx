...
const ShowLibraryCard = (props: Props) => {
  const { delay } = props;
  ...
  const saveList = () => {
    ...
    newSettings.showLibraries = selectedLibraries;
    dispatch(saveSettings(newSettings));
    SnackbarUtils.info(t('SETTINGS.SHOW.LIBRARY.SAVING'));
  }
  ...
  return (
    ...
    >
      <Grid item>
        <Typography>
          {t('SETTINGS.SHOW.COLLECTIONTYPES', { type: settings.mediaServer.serverType === 0 ? "Emby" : "Jellyfin" })}
        </Typography>
      </Grid>
      <Loading
        className="m-t-32"
        ...
        libraries={settings.showLibraries}
...
export default ShowLibraryCard
