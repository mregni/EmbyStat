...
import { makeStyles, ListItem, ListItemText, Collapse, RadioGroup, Button, Grid } from '@material-ui/core';
...
import { FilterDefinition, ActiveFilter, FilterType } from '../../../models/filter';
...
const FilterPicker = (props: Props) => {
  ...
  const calculateValue = (type: string): string | number => {
    switch (filterDefinition.field) {
      ...
      case "Genres":
      case "Container":
      case "Subtitles": return `${value.split('|')[0]}`;
      default: return encodeURIComponent(value);
    }
  }
  ...
  const calculateValueLabel = (type: FilterType): string => {
    ...
    switch (type.type) {
      ...
      case "dateRange": return `${encodeURI(moment(value.split('|')[0]).format('L'))} ${t('COMMON.AND')} ${encodeURI(moment(value.split('|')[1]).format('L'))}`;
      default: break;
    }

    switch (type.operation) {
      case "between": return `${value.split('|')[0]}${type.unit != null ? t(type.unit) : ''} ${t('COMMON.AND')} ${value.split('|')[1]}${type.unit != null ? t(type.unit) : ''}`;
      case "null": return '';
      default: return `${value}${type.unit != null ? t(type.unit) : ''}`;
    }
  }
  ...
  return (
    <>
      <ListItem button onClick={handleClick}>
        <ListItemText primary={filterDefinition.label.replace(/ \{[0-9]\}/g, '')} />
            ...
            <RadioGroup name="type">
              {
                types.map(type =>
                  ...
                  >
                    <>
                      {type.type === "txt" ? <FilterTextField
                        type={type}
                        onValueChanged={setValue}
                        errors={errors}
                        register={register}
                        disableAdd={setIntputInError} /> : null}
                      {type.type === "number" ? <FilterNumberField
                        type={type}
                        onValueChanged={setValue}
                        errors={errors}
                        register={register}
                        disableAdd={setIntputInError} /> : null}
                      {type.type === "range" ? <FilterBetweenField
                        type={type}
                        onValueChanged={setValue}
                        errors={errors}
                        register={register}
                        disableAdd={setIntputInError} /> : null}
                      {type.type === "dropdown" ? <FilterDropdownField
                        type={type}
                        onValueChanged={setValue}
                        field={filterDefinition.field}
                        disableAdd={setIntputInError} /> : null}
                      {type.type === "date" ? <FilterDateField
                        onValueChanged={setValue}
                        errors={errors}
                        register={register}
                        disableAdd={setIntputInError} /> : null}
