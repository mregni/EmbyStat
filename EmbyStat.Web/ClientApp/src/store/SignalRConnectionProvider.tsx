  ...
  componentDidMount() {
    if (this.state === null) {
      ...
      this.setState({ hubConnection: connection }, () => {
        ...
        this.state.hubConnection.on('JobReportProgress', this.JobReportProgressReceived);
        this.state.hubConnection.on('JobReportLog', this.onJobReportLogReceived);
        this.state.hubConnection.on('MediaServerConnectionState', this.onNotifReceived);
        this.state.hubConnection.on('UpdateState', this.onNotifReceived);
      });
    }
  }

  startSignalRConnection = connection => connection
    .start()
