...
namespace EmbyStat.Tasks
{
    ...
    public class SystemUpdateTask : IScheduledTask
    {
        ...
        public SystemUpdateTask(IApplicationHost appHost, IConfigurationManager configurationManager)
        {
            _appHost = appHost;
        ...
        public async Task Execute(CancellationToken cancellationToken, IProgress<double> progress)
        {
            ...
            {
                Log.Debug("No application update available.");
                return;
            }
            ...
            {
                Log.Information("Update Revision {0} available.  Updating...", updateInfo.AvailableVersion);

                await _appHost.UpdateApplication(updateInfo.Package, cancellationToken, progress).ConfigureAwait(false);
            }
            else
            {
                Log.Information("A new version of " + _appHost.Name + " is available.");
            }
        }
